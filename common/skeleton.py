...
class Skeleton(object):
    ...
    def forward_kinematics_hand_cont6d(
        self,
        cont6d_params,
        hand_joints=15,
        wrist_index=20,
        ...
        # joints[..., 0, :] = root_pos
        for chain in self._kinematic_tree:
            if chain[0] != wrist_index:
                continue
            if do_root_R:
                matR = cont6d_to_matrix(cont6d_params[:, wrist_index])
            else:
                matR = (
                    torch.eye(3)
                    .expand((len(cont6d_params), -1, -1))
            ...
            for i in range(1, len(chain)):
                ...
                # torch.matmul(matR, cont6d_to_matrix(cont6d_params[:, chain[i]]))
                offset_vec = offsets[:, chain[i]].unsqueeze(-1)
                # print(matR.shape, offset_vec.shape)
                joints[:, chain[i]] = (
                    torch.matmul(matR, offset_vec).squeeze(-1) + joints[:, chain[i - 1]]
